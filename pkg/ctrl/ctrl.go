...
func (c *BaseController) PluginUpdate(ctx context.Context) error {
	...
	// Generate workspace dependency lockfile
	workspaceLock, err := c.GenerateWorkspaceLockfile()
	if err != nil {
		return fmt.Errorf("failed to generate workspace lockfile: %w", err)
	}

	if len(workspaceLock.Dependencies) > 0 {
		if err := c.SaveLockfile(workspaceLock); err != nil {
			return fmt.Errorf("failed to save workspace lockfile: %w", err)
		}
		fmt.Printf("✅ Generated workspace lockfile with %d dependencies\n", len(workspaceLock.Dependencies))
	} else {
		fmt.Println("✅ No workspace dependencies detected")
	}
	fmt.Println("✅ All plugins updated successfully")
