#!/usr/bin/env bash
# PROTOTYPE — builds the nexus-host:latest image from the repo.
#
# Build context is the nexus repo root so the Go source and images/kernel/*
...
PROTO_DIR="$REPO_ROOT/examples/nexus-in-docker"
...
echo "Building nexus-host:latest from repo root …"
docker build \
...
echo ""
echo "OK -> nexus-host:latest"
echo "Image size: $(docker image inspect nexus-host:latest --format '{{.Size}}' | numfmt --to=iec 2>/dev/null || docker image inspect nexus-host:latest --format '{{.Size}}')"
