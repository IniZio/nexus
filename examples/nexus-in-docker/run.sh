#!/usr/bin/env bash
# PROTOTYPE — run the nexus host daemon inside a container.
#
...
#   If you bind-mount the HOST's ~/.local/state/nexus the disk images are owned
...
#   nexus has two socket-dir formulas: the CLI uses $TMPDIR, the CHDriver
...
#   in /tmp/nexus-0/ and every exec fails with "no such file or directory".
set -euo pipefail

IMAGE="${NEXUS_IMAGE:-nexus-host:latest}"
