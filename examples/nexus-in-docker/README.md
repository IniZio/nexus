# Example: nexus host daemon in a Docker container

Demonstrates running the nexus **host daemon** inside a Docker container so
that it can boot microVMs — the same deployment shape microsandbox uses for its
`msbserver` image.

**Proven end-to-end (Aug 2026, docker 29.5.3):**

1. `nexus doctor` — all 5 capability checks pass with `--device /dev/kvm`;
   the `kvm` check FAILs without it.
2. `nexus create --file` — pulled `moby/buildkit`, assembled the builder ext4,
   booted the builder microVM, ran buildkit, and produced the image.
3. `nexus exec` in the guest returned `/marker` and `uname -sr`:
...
| glibc base image | `nexus` is glibc-dynamic (CH + virtiofsd are static-pie) |
| kernel via `NEXUS_KERNEL_PATH` | `images/kernel/vmlinux-x86_64` baked in |
...
Mount a fresh container-owned Docker volume at `/root/.local/state/nexus`.
If you bind-mount the host's `~/.local/state/nexus`, disk images are owned by
...
nexus has two socket-directory formulas that diverge when `$TMPDIR` is set:
...
Without this, `create` puts sockets in `/work/rt/nexus/` but `exec` looks
in `/tmp/nexus-0/` → every exec fails with "no such file or directory".
...
| Binary | Source |
|---|---|
| `nexus` | Built from source in the Go builder stage (`go build ./cmd/nexus`) |
...
- Tagged: `nexus-host:latest`

## Build

```bash
# From nexus repo root — stages virtiofsd from host then builds the image:
bash examples/nexus-in-docker/build.sh
```

`build.sh` copies the host `virtiofsd` binary into `examples/nexus-in-docker/stage/`
...
# Full boot (fresh container-owned state so nexus owns its disks):
...
docker run --rm \
    ...
    -v n3state:/root/.local/state/nexus \
    -v n3work:/work \
    --entrypoint sh nexus-host:latest -c '
...
```bash
# Proves nexus compiles correctly inside the image build:
docker build -f examples/nexus-in-docker/Dockerfile --target go-builder -t n3-smoke-go .

# Proves the cloud-hypervisor download URL + SHA-256 pin still resolve:
docker build -f examples/nexus-in-docker/Dockerfile --target downloader -t n3-smoke-dl .
```
