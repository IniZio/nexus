...
const (
	...
	agentRef            = "nexus-agent-base"
	agentDockerTag      = "nexus-agent-base:integration-test"
...
func BuildAgentBaseImage(ctx context.Context, cache *image.Cache) (domain.Image, error) {
	...
	workDir, err := os.MkdirTemp("", "nexus-agent-image-build-*")
	if err != nil {
...
func generateAgentContainerfile(goVer, goSHA256 string, srcDirs []string) string {
	...
	return fmt.Sprintf(`# nexus agent base image
...
# + git + curl + ca-certs + nexus-agent + seeded Go module cache.
#
...
# ca-certificates (TLS) + iproute2 (ip link/addr/route for nexus-agent network init
...
# guest: the VM boots init=/sbin/nexus-agent directly from the ext4 rootfs, so
# no container runtime ever reads Config.Env.  nexus-agent reads /etc/environment
...
RUN printf '%%s=%%s\n' \
        ...
        PATH "$PATH" \
    > /etc/environment && \
    { echo '# nexus: Go toolchain env — generated from the Containerfile ENV block.'; \
      echo '# Do not edit; edit the ENV declarations and rebuild.'; \
      sed 's/^/export /' /etc/environment; } \
    > /etc/profile.d/nexus-go.sh && \
    chmod 0644 /etc/environment /etc/profile.d/nexus-go.sh && \
    mkdir -p "$GOMODCACHE"
