...
func TestSeedLoop_GuestReachability(t *testing.T) {
	...
	t.Run("guest_never_reachable", func(t *testing.T) {
		...
		ok, guestEverResponded := supervisor.SeedLoop(context.Background(), id, &c,
			failCA, okAgent, broker, nil, 3, 0, nil, true, cred.MustProfileByName(cred.ClaudeCodeProfileName), nil, nil)
		if ok {
	...
	t.Run("guest_alive_seed_incomplete", func(t *testing.T) {
		...
		c := cert
		ok, guestEverResponded := supervisor.SeedLoop(context.Background(), id, &c,
			okCA, failAgent, broker, nil, 3, 0, nil, true, cred.MustProfileByName(cred.ClaudeCodeProfileName), nil, nil)
		if ok {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// nil svc is safe: cert is non-nil, so GetPerimeterCACert is never called.
				...
				caSeeder, agentSeeder, cred.NewBroker(), nil, 3, 0, nil, tc.seedAgentCreds, cred.MustProfileByName(cred.ClaudeCodeProfileName), nil, nil)
