#!/bin/sh
# nexus-runc: OCI runtime shim baked into every `--file` guest image at
...
# read-only at /etc/nexus/ca plus the common TLS env vars pointing at it, so
# TLS-intercepted hosts verify without any Dockerfile change. Inert until the
# supervisor has seeded the MITM CA (NEXUS_CA_CRT). It never touches
...
CA_CRT="${NEXUS_CA_CRT:-/usr/local/share/ca-certificates/nexus-mitm.crt}"
SYS_BUNDLE="${NEXUS_SYS_BUNDLE:-/etc/ssl/certs/ca-certificates.crt}"
TRUST_DIR="${NEXUS_TRUST_DIR:-/etc/nexus/docker-trust}"
CA=/etc/nexus/ca/ca-certificates.crt
...
if [ -n "$cmd" ] && [ -f "$CA_CRT" ] && [ -f "$SYS_BUNDLE" ]; then
	...
	if [ -f "$cfg" ] && ! grep -q '"destination":"/etc/nexus/ca"' "$cfg" && prepare_trust_dir; then
		ENVS="\"SSL_CERT_FILE=$CA\",\"CURL_CA_BUNDLE=$CA\",\"REQUESTS_CA_BUNDLE=$CA\",\"PIP_CERT=$CA\",\"NODE_EXTRA_CA_CERTS=$CA\",\"GIT_SSL_CAINFO=$CA\",\"CARGO_HTTP_CAINFO=$CA\",\"NIX_SSL_CERT_FILE=$CA\",\"DENO_CERT=$CA\",\"WGETRC=/etc/nexus/ca/wgetrc\",\"APT_CONFIG=/etc/nexus/ca/apt.conf\""
		MOUNT="{\"destination\":\"/etc/nexus/ca\",\"type\":\"bind\",\"source\":\"$TRUST_DIR\",\"options\":[\"rbind\",\"ro\",\"nosuid\",\"nodev\"]}"
		...
		tr -d '\n' <"$cfg" >"$cfg.nexus" 2>/dev/null &&
			sed -i \
				-e "s|\"mounts\":[[:space:]]*\\[|\"mounts\":[$MOUNT,|" \
				-e "s|\"env\":[[:space:]]*\\[|\"env\":[$ENVS,|" \
				-e 's|,[[:space:]]*\]|]|g' "$cfg.nexus" 2>/dev/null &&
			mv -f "$cfg.nexus" "$cfg" 2>/dev/null || rm -f "$cfg.nexus"
	fi
fi
