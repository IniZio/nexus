...
// TestProbeAndSeedGuest_SymlinksOnlyManifestSeeded is the mutation guard for
// the no-op guard change in SeedGuestUserMounts (len(Mounts)==0 &&
// len(Symlinks)==0). When a manifest carries ONLY Symlinks (no Mounts),
// seedUserMountsFn must still be invoked so the guest symlinks are created.
//
//	Revert guard to `len(Mounts)==0` alone → service.SeedGuestUserMounts becomes
//	a no-op; guest symlinks are never created.
func TestProbeAndSeedGuest_SymlinksOnlyManifestSeeded(t *testing.T) {
	called := false
	old := seedUserMountsFn
	seedUserMountsFn = func(_ context.Context, _ domain.SandboxID, _ service.UserMountManifest, _ service.GuestExecer) error {
		called = true
	...
	manifest := service.UserMountManifest{
		HostHome: "$HOME",
		Mounts:   nil, // no mounts — symlinks only
	...
	if !called {
		t.Fatal("seedUserMountsFn was not called for a Symlinks-only manifest — " +
			"guest symlinks will never be created when no Mounts rows are present")
	}
}
