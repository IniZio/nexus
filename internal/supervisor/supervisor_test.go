...
func TestBuildSupervisorDriverConfig_BalloonModeBootAndCeiling(t *testing.T) {
	cfg := Config{
		CHBin:      "/usr/bin/cloud-hypervisor",
		SocketDir:  "/run/user/1000/n3",
		KernelPath: "/k",
	...
	if got.MemoryMiB != 2048 {
		t.Errorf("MemoryMiB = %d, want 2048 (boot RAM)", got.MemoryMiB)
	}
	if got.MemoryMaxMiB != 8192 {
		t.Errorf("MemoryMaxMiB = %d, want 8192 (ceiling)", got.MemoryMaxMiB)
	}
	if !got.FreePageReporting {
