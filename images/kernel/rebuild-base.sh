#!/usr/bin/env bash
# rebuild-base.sh — rebuild the nexus agent binary in images/kernel/
#                   and optionally rebuild the nexus-agent-base ext4 image.
#
# Usage:
...
#
# The agent binary (CGO_ENABLED=0) is written to images/kernel/nexus-agent.
...
#   • cmd/nexus-agent/**  (agent source code)
#   • internal/core/agent/**  (agent library code)
# Run it with --image to also update the nexus-agent-base image cache entry.
# Without --image, only images/kernel/nexus-agent is updated; sandboxes that
# use --rootfs will pick up the new binary immediately.  Sandboxes that use
# --image nexus-agent-base still need the full image rebuild.
#
# Staleness detection
# ───────────────────
...
BINARY_SIZE=$(stat -c%s images/kernel/nexus-agent)
echo "nexus-agent: built → images/kernel/nexus-agent (${BINARY_SIZE} bytes, build=${BUILD_TAG})"
...
if [[ "$REBUILD_IMAGE" == "true" ]]; then
  ...
  echo "nexus-agent-base: rebuilding full image via cmd/rebuild-agent-base..."
  ...
  echo ""
  echo "nexus-agent-base: image rebuild complete."
  echo "The new image is in the production cache. To ensure it is resolved first"
  echo "by 'sandbox create --image nexus-agent-base', remove stale entries:"
  echo ""
  echo "  go run ./cmd/nexus image ls  # see all cached images"
  echo ""
  echo "Stale entries (ref=nexus-agent-base, created 2026-08-11) can be removed"
  echo "by deleting their sha256/ subdirectories from ~/.local/state/nexus/images/"
else
  echo ""
  echo "Agent binary rebuilt. To also rebuild the nexus-agent-base image, run:"
