...
| **Default base** | `images/base/Containerfile` → `ghcr.io/inizio/nexus3-base:<tag>` | `debian:bookworm-slim` with e2fsprogs, openssh-server, git, iproute2, ca-certificates. Published by CD; used by default when no `.nexus/Containerfile` is present. Override with `.nexus/Containerfile`. |

`nexus3-agent-base` (generated by `generateAgentContainerfile` in `internal/test/selfhost/baseimage_agent.go`) is **dev-only** — the full self-hosting base (Go + Node.js + Claude Code + gh + seeded module cache) built by `go run ./cmd/rebuild-agent-base`. A fresh user host cannot satisfy it; the herdr plugin uses `ghcr.io/inizio/nexus3-base` instead.
...
Every `exec.Command` call in `cmd/nexus3-agent/` and the worktree flow that determines the package set:
...
| `/usr/sbin/sshd` | `sshd_start.go:57` | `openssh-server` (consumer: `nexus3 ssh --stdio <ref>`) |
...
`images/base/Containerfile` builds `ghcr.io/inizio/nexus3-base`. It does NOT build `nexus3-agent-base` — that image is generated by `generateAgentContainerfile()` in `internal/test/selfhost/baseimage_agent.go`.
