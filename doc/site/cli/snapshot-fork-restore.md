...
`fork` and `restore` are the verbs that distinguish nexus: each call produces one new running child from a copy-on-write parent, which is what makes isolated agents cheap enough to be routine. See [Snapshots and fork](/sandboxes/snapshots-and-fork) for the conceptual model.
...
```
nexus snapshot create <sandbox-ref> [<name>]
```
