...
nexus owns zero OCI code. The image format is ext4; the build input is a standard Dockerfile/Containerfile — no custom syntax. The agent binary is baked as the final layer, ensuring agent and host versions always match.

```sh
nexus image build --workspace . --ref my-app:latest
nexus image ls
nexus create my-app --image my-app:latest
```
...
The agent binary (`nexus-agent`) is baked as the **final layer** of every image. This prevents version skew between the in-guest agent and the nexus host binary.

Images may also bake a **startup services table** at `/etc/nexus/services.yaml`. The guest agent reads this table at boot and starts declared services before accepting connections, making `nexus create` readiness-gated. <Badge type="danger" text="not built" /> See [Guest agent — startup services](guest-agent.md#startup-services).

## Build path

```mermaid
flowchart TD
    A["<b>nexus image build</b><br/>--workspace DIR --ref TAG"] --> B
    B["Builder VM<br/>nexus-agent --builder-role<br/>dedicated netns · unrestricted egress<br/>cache disks mounted at designated paths"] --> C
    C["BuildKit<br/>(buildkitd in-guest, standard upstream)"] --> D
    D["OCI image layers"] --> E
    E["nexus wraps layers<br/>→ ext4 base image"] --> F
...
The builder VM uses the same `nexus-agent` binary in `--builder-role` mode. In this role the agent runs the BuildKit lifecycle and exits; it does not start the gRPC control server.
...
| Image | Contents |
|-------|----------|
| **Base sandbox image** | Minimal Linux userland + `nexus-agent` as PID 1. Used by sandboxes that don't supply a `.nexus/Containerfile`. |
| **Agent rebuild image** | The toolchain needed to rebuild `nexus-agent` itself. Used by the self-hosting build path. |

Every project that needs additional software commits a `.nexus/Containerfile` that extends the base image.

## Guest kernel

The guest kernel is shipped alongside the nexus binary, per-arch. It is not part of the OCI image; it is a separate artifact managed by the nexus host, so the kernel can be upgraded independently of the rootfs image.
...
The agent rebuild image contains the Go toolchain and the nexus source tree. Build path:

1. Launch a sandbox from the rebuild image.
2. Inside the sandbox, `go build ./cmd/nexus-agent` produces a new agent binary.
...
An image may bake a services table at `/etc/nexus/services.yaml`. The guest agent reads this at boot and starts all declared services before opening the vsock control port, making `nexus create` readiness-gated.

```dockerfile
# .nexus/Containerfile
FROM nexus-base
RUN apt-get install -y docker.io
COPY services.yaml /etc/nexus/services.yaml
```
...
Image-level declarations form the base; create-time `--service` flags on `nexus create` can override a same-named entry <Badge type="danger" text="not built" />. See [Guest agent — startup services](guest-agent.md#startup-services) for the full semantics.
