...
> Declare dockerd as a readiness-gated startup service so `nexus create` blocks until Docker is ready — no manual start, no poll loop.
...
nexus sandboxes are workload-agnostic Linux VMs. Install Docker into the guest image, declare it
as a **startup service** <Badge type="danger" text="not built" />, and `nexus create` blocks
...
- `--storage-driver=overlay2` — the nexus guest kernel supports overlayfs.

`ready: [docker, info]` is the readiness probe. `nexus create` polls it and returns only once
it exits zero, with a 30-second cap after which `create` fails. There is no poll loop to write.

The `.nexus/services.yaml` baked into the image is the canonical service declaration. The `--service 'name:cmd[:probe]'` flag on `nexus create` is a per-sandbox addition or override — it takes precedence over a same-named image entry without requiring an image rebuild. Image-level entries that are not overridden are unchanged. <Badge type="danger" text="not built" />

**3. Create the sandbox** <Badge type="danger" text="not built" />

```sh
nexus create myproject/my-sandbox --context .
# blocks ~2 s, prints "starting dockerd... ready"

nexus exec myproject/my-sandbox -- docker ps
...
```sh
nexus create myproject/my-sandbox \
  --context . \
...
```sh
# Single container
nexus exec myproject/my-sandbox -- docker run --rm hello-world

# docker compose (compose.yml must be present in the guest workspace)
nexus exec myproject/my-sandbox -- docker compose up -d
```
...
`nexus create --nested` enables `/dev/kvm` passthrough so the guest can run nested VMs.
...
- `nexus create` blocks until the `docker info` probe passes. Socket readiness is observed at
...
- Default-deny egress is enforced by the nexus host perimeter. Running dockerd inside the
  sandbox does not change the sandbox's outbound network policy — all traffic still flows through
  the nexus MITM perimeter.
- Do **not** bake credentials (registry auth, secret env vars) into the guest image via the
  Containerfile. Inject secrets at runtime via `nexus exec` environment variables or the
  credential broker.
- Docker's guest-side bridge/iptables NAT is independent of the nexus perimeter. The perimeter intercepts all outbound traffic at the gvproxy gateway (network-stack layer on the host), which is orthogonal to guest iptables — Docker's default NAT does not bypass or conflict with the perimeter allowlist.

---

## Design rationale <Badge type="danger" text="not built" />

`nexus create` is readiness-gated: it returns only once every declared startup service has
...
The one exception is `nexus create --context`, which starts an in-VM **buildkitd** to build the
custom guest image. That is a nexus primitive, not user-workload Docker.
