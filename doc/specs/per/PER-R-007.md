...
The persistent-perimeter implementation **shall not** add the non-essential old-nexus (the predecessor) vsock proxies (ssh-agent, git-signing, docker-cred, notification-relay, PTY-host) to the supervisor, **shall not** place any real credential token on the guest disk, and **shall** leave all non-orca / non-`CreateAndBoot`-supervised sandbox flows with behaviour identical to the pre-perimeter baseline.
