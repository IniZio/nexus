...
**When** `nexus3-client` starts, it **shall** cancel any forwards that were applied by a previous client process and are no longer present in the current reconciliation set before applying new forwards; and **when** the host port-forward state file contains duplicate rows for the same sandbox port from both `tcp` and `tcp6` protocol entries, `nexus3-client` **shall** collapse them to a single forward entry.
