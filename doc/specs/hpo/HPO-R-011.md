...
1. `clientagent.Tick` **shall** derive the focused workspace from `~/.local/state/nexus3/portfwd/focus.state`, a file maintained by the host-side focus watcher goroutine via three sources: snapshot seed at startup, `events.subscribe` API stream, and server log tail (`herdr-server.log` polled every 250ms for lines with `event="workspace.focus"` and `outcome="ok"`). On herdr 0.9.0 the server log tail is the only source that captures Mac remote-client focus changes, because remote-client focus is not reflected in `session.snapshot`, `workspace.list`, `events.subscribe`, or any plugin hook. Focus-scoped forwarding is accurate when the remote `nexus3-client` is the **sole interactive client** on that herdr session; when both a host TUI and a remote client are attached, the host session's last write wins.
