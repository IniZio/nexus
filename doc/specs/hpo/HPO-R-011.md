...
1. `clientagent.Tick` **shall** derive the focused workspace from `~/.local/state/nexus/portfwd/focus.state`, a file maintained by the host-side focus watcher goroutine via three sources: snapshot seed at startup, `events.subscribe` API stream, and server log tail (`herdr-server.log` polled every 250ms for lines with `event="workspace.focus"` and `outcome="ok"`). On herdr 0.9.0 the server log tail is the only source that captures Mac remote-client focus changes, because remote-client focus is not reflected in `session.snapshot`, `workspace.list`, `events.subscribe`, or any plugin hook. Focus-scoped forwarding is accurate when the remote `nexus-client` is the **sole interactive client** on that herdr session; when both a host TUI and a remote client are attached, the host session's last write wins.

2. `nexus herdr doctor` **shall** detect and report duplicate herdr server processes, remote-client-bridge processes, and `nexus-client` daemon processes with a human-readable remediation command, and **shall not** kill any process it does not own (i.e., it **shall not** terminate herdr server or remote-client-bridge processes from plugin code).

3. The macOS `[[startup]]` hook (`nexus-client herdr local-agent-startup`) **shall** kill at most one previous `nexus-client` instance — the one whose PID is recorded in the plugin's own pidfile under `$HERDR_PLUGIN_STATE_DIR` — and **shall not** kill herdr-owned processes.
...
- **Fit criterion** — (a) `focus.state` is updated within one reconcile tick of a workspace focus change (via watcher goroutine or `workspace.focused` hook) and read by `clientagent.Tick` without issuing any `herdr workspace list` call; (b) `nexus herdr doctor` output includes entries for each duplicate class with a remediation command, and the implementation contains no `SIGKILL`/`SIGTERM` directed at herdr server or bridge pids; (c) a second `nexus-client herdr local-agent-startup` invocation kills the pid from the existing pidfile and no other process.
