...
**When** `herdr plugin install IniZio/nexus/plugins/herdr` completes on Linux, the install **shall** run `nexus doctor` and surface any failing check with its remediation text; **if** a check fails, the install **shall** register the plugin and print a warning rather than aborting, so that the doctor pane and plugin actions remain reachable; **when** the user subsequently runs `nexus herdr worktree-sandbox` or `nexus create`, the system **shall** re-run the preflight and fail fast with the same remediation text instead of producing a late VM-boot error.
...
- **Fit criterion** — on a host where the `kvm` check fails: (a) install exits 0 and `herdr plugin list` shows the plugin enabled; (b) install output contains the check name and its remediation string; (c) `nexus herdr worktree-sandbox` exits non-zero within the first reconcile tick and its stderr contains the same remediation string without attempting VM boot.
