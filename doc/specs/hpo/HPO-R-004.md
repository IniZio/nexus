...
**When** `nexus3 herdr doctor` or the plugin `[[startup]]` hook detects that the installed `nexus3` binary version or herdr ABI does not match the pinned release, the system **shall** print the exact command `run: herdr plugin install IniZio/nexus3/plugins/herdr` verbatim in its output.
...
- **Fit criterion** — with the installed binary reporting a version or ABI that differs from the expected pin, both `nexus3 herdr doctor` stdout and the `[[startup]]` hook output contain the literal string `herdr plugin install IniZio/nexus3/plugins/herdr`. A test asserts the literal presence of this string.
