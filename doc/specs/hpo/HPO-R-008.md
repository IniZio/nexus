...
- **Constraint (D-14)** — herdr 0.9.0 protocol 22 carries no client identity: `workspace_focused` delivers `{type, workspace_id}` with no client field, the `api snapshot` exposes a single session-global `focused_workspace_id`, and pane env variables (`HERDR_PANE_ID`, `TAB_ID`, `WORKSPACE_ID`, `SESSION`) include no client id. On herdr 0.9.0, Mac remote-client focus changes are not reflected in `session.snapshot`, `workspace.list`, `events.subscribe`, or any plugin hook; the focus watcher therefore reads the session server log (`herdr-server.log`) for lines with `event="workspace.focus"` and `outcome="ok"` as a third source alongside the snapshot seed and `events.subscribe` stream. The focused workspace is derived from a `focus.state` file written by that watcher goroutine and read by `clientagent.Tick` over ssh — not from a per-client API. This is accurate only when the remote client is the **sole interactive client** on the herdr session; when a host TUI and a remote client coexist, the host session's last-write wins.
