...
Covers the `nexus-mount-creds-ssh-relay` motive: live virtiofs mount of the host `~/.claude` directory, CredGuardian proactive refresh, auto permission mode (no `--dangerously-skip-permissions`), git over SSH via vsock relay with policy guard and branch enforcement, brokered `GH_TOKEN` regression guard, and port auto-forward with herdr ≥ 0.9.
