...
- **Tests** `TestSeedGuestAgent_ClaudeVarsPresentRealTokenAbsent` (`internal/core/service/seed_agent_test.go:13`) — asserts `CLAUDE_CODE_OAUTH_TOKEN=` present with a broker placeholder and real token structurally absent from the guest payload; `TestSeedLoop_ForcePushWritesRealToken` (`internal/supervisor/supervisor_combined_test.go`); acceptance layer: `TestSupervisorS4ClaudeCodePlaceholderInGuest` and `TestSupervisorS4ClaudeCodeLiveSubstitution` (`internal/test/selfhost/supervisor_s4_test.go`)
