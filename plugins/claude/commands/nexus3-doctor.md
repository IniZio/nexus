# /nexus3:nexus3-doctor

Report the real state of the nexus3 substrate, the MCP connection, and running
...
Check whether the nexus3 MCP server is connected by calling the `sandbox_list`
...
```bash
grep -A3 '"nexus3"' ~/.claude.json
```

If that binary is not `~/.local/bin/nexus3`, the server may be running a stale
...
`nexus3 ps` output is an orphan (herdr binding with no live VM).
