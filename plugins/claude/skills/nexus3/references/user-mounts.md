...
nexus3 has **no built-in default mounts** — it ships no hardcoded tool list.
...
The file lives at `$XDG_CONFIG_HOME/nexus3/config.yaml` (falls back to
`~/.config/nexus3/config.yaml` when `$XDG_CONFIG_HOME` is unset).
