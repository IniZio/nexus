...
name: nexus
description: >
  Load for anything involving nexus microVM sandboxes. (1) Sandbox operations:
  ...
  (2) First-run onboarding of a repo that has never used nexus: detecting the
...
| User-global config (`~/.config/nexus/config.yaml`), diagnose missing tools, security boundary | `references/user-mounts.md` |
| First-run onboarding: detect stack, author `.nexus/config.yaml` + `.nexus/Containerfile`, explain where egress config is read from (`/nexus:nexus-init`) | `references/onboard.md` |
