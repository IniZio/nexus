...
- `chmod` on files the guest created applies the requested mode minus g/o+w (`req & 0o755`); on pre-existing host files exec bits pass through and rw bits can only be removed. Invariant: `host_mode & 0o022 == 0` unless the host set it. `chown` is no-oped; `access()` always succeeds.
