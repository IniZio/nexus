...
func bootstrapContainerExecContext(projectRoot string, execCtx doctorExecContext, backendLabel string, allowInstall bool) error {
	...
	startDockerCmd := `mkdir -p /sys/fs/cgroup; if ! grep -q ' /sys/fs/cgroup ' /proc/mounts; then mount -t cgroup2 none /sys/fs/cgroup 2>/dev/null || mount -t tmpfs tmpfs /sys/fs/cgroup || true; fi; if ! grep -q ' /sys/fs/cgroup cgroup2 ' /proc/mounts; then for s in cpuset cpu cpuacct blkio memory devices freezer net_cls perf_event net_prio hugetlb pids; do mkdir -p /sys/fs/cgroup/$s; mount -t cgroup -o $s cgroup /sys/fs/cgroup/$s 2>/dev/null || true; done; fi; if command -v iptables-legacy >/dev/null 2>&1; then ln -sf "$(command -v iptables-legacy)" /usr/sbin/iptables >/dev/null 2>&1 || true; fi; if command -v ip6tables-legacy >/dev/null 2>&1; then ln -sf "$(command -v ip6tables-legacy)" /usr/sbin/ip6tables >/dev/null 2>&1 || true; fi; if command -v iptables-legacy >/dev/null 2>&1 && command -v update-alternatives >/dev/null 2>&1; then update-alternatives --set iptables /usr/sbin/iptables-legacy >/dev/null 2>&1 || true; update-alternatives --set ip6tables /usr/sbin/ip6tables-legacy >/dev/null 2>&1 || true; fi; if command -v modprobe >/dev/null 2>&1; then modprobe iptable_raw >/dev/null 2>&1 || true; modprobe iptable_nat >/dev/null 2>&1 || true; modprobe xt_addrtype >/dev/null 2>&1 || true; fi; if command -v systemctl >/dev/null 2>&1; then systemctl enable docker >/dev/null 2>&1 || true; systemctl start docker >/dev/null 2>&1 || true; fi; mirror_flag=''; if [ -n "${NEXUS_DOCKER_REGISTRY_MIRROR:-}" ]; then mirror_flag="--registry-mirror=${NEXUS_DOCKER_REGISTRY_MIRROR}"; else mirror_flag='--registry-mirror=https://mirror.gcr.io'; fi; raw_optout_env=''; if [ "${NEXUS_DOCKER_DISABLE_IPTABLES_RAW:-1}" = "1" ]; then raw_optout_env='DOCKER_INSECURE_NO_IPTABLES_RAW=1'; fi; if ! docker info >/dev/null 2>&1; then mkdir -p /workspace/.nexus-docker; rm -rf /var/lib/docker/* /var/lib/containerd/* 2>/dev/null || true; nohup sh -lc "exec env ${raw_optout_env} dockerd --host=unix:///var/run/docker.sock --data-root=/workspace/.nexus-docker --exec-root=/workspace/.nexus-docker-exec --storage-driver=overlay2 --userland-proxy=false --allow-direct-routing ${mirror_flag}" >/tmp/nexus-doctor-dockerd.log 2>&1 & sleep 5; if ! docker info >/dev/null 2>&1 && grep -q 'unknown flag: --allow-direct-routing' /tmp/nexus-doctor-dockerd.log 2>/dev/null; then nohup sh -lc "exec env ${raw_optout_env} dockerd --host=unix:///var/run/docker.sock --data-root=/workspace/.nexus-docker --exec-root=/workspace/.nexus-docker-exec --storage-driver=overlay2 --userland-proxy=false ${mirror_flag}" >/tmp/nexus-doctor-dockerd.log 2>&1 & sleep 5; fi; fi; if [ -n "${NEXUS_DOCKERHUB_USERNAME:-}" ] && [ -n "${NEXUS_DOCKERHUB_TOKEN:-}" ]; then echo "$NEXUS_DOCKERHUB_TOKEN" | docker login -u "$NEXUS_DOCKERHUB_USERNAME" --password-stdin docker.io >/tmp/nexus-docker-login.log 2>&1 || true; fi`
...
func runDoctorLifecycleSetup(projectRoot string, execCtx doctorExecContext) error {
	...
	if setupExists, err := isExecutableFile(setupPath); err != nil {
		...
		command = "sh"
		args = []string{"-lc", "if [ -f .tool-versions ] && command -v mise >/dev/null 2>&1; then mise install >/dev/null 2>&1 || true; eval \"$(mise activate bash)\" || true; fi; if command -v corepack >/dev/null 2>&1 && ! corepack --version >/dev/null 2>&1; then rm -f \"$(command -v corepack)\" || true; fi; if command -v corepack >/dev/null 2>&1; then corepack enable >/dev/null 2>&1 || true; corepack prepare pnpm@latest --activate >/dev/null 2>&1 || true; fi; if command -v pnpm >/dev/null 2>&1 && ! pnpm --version >/dev/null 2>&1; then rm -f \"$(command -v pnpm)\" || true; fi; if ! command -v pnpm >/dev/null 2>&1 && command -v npm >/dev/null 2>&1; then npm install -g pnpm >/dev/null 2>&1 || true; fi; if ! command -v pnpm >/dev/null 2>&1 && command -v corepack >/dev/null 2>&1; then mkdir -p /usr/local/bin; printf '#!/bin/sh\nexec corepack pnpm \"$@\"\n' > /usr/local/bin/pnpm; chmod 755 /usr/local/bin/pnpm; fi; make setup"}
