...
export class ValidationEngine {
  ...
  validateDogfooding(workspaceInfo: WorkspaceInfo, rules: EnforcerRules['dogfooding'], frictionLogPath?: string): FailedCheck[] {
    const failures: FailedCheck[] = [];
  ...
  validate(context: ExecutionContext): ValidationResult {
    ...
    const totalChecks = allFailures.length || 1;
