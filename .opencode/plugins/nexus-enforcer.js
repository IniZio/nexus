...
class BoulderEnforcer {
    ...
    state;
    cooldownActive;
    enforcementTriggeredForThisIdlePeriod;
    toolInProgress;
    permissionPending;
    directory;
    ...
    checkText(text) {
        ...
        const hasCompletion = this.config.completionKeywords.some(keyword => lower.includes(keyword.toLowerCase()));
        if (!hasCompletion)
            return false;
        const hasWorkIndicators = this.config.workIndicators.some(indicator => lower.includes(indicator.toLowerCase()));
...
async function triggerEnforcementWithCountdown(directory, client, enforcerInstance, log, reason = 'idle', sessionID = null) {
    if (enforcementLock) {
...
function startPolling(directory, client, log) {
    if (pollInterval) {
...
function isMainAgent(input) {
    ...
    if (input.isSubAgent === true)
        return false;
    if (input.agentType === 'sub' || input.agentType === 'child')
        return false;
    if (input.parentSession && input.parentSession !== input.session)
        return false;
    return true;
}
...
export default async function NexusOpenCodePlugin(context) {
        ...
        tool: {
            nexus_boulder_pause: tool({
                ...
                async execute(_args, ctx) {
                    const statePath = path.join(ctx.directory, '.nexus/boulder/state.json');
