...
//
// When nexus-agent runs as PID 1 (in-guest init), it configures the guest
// virtio-net interface with a static IP and DNS server.  The nexus perimeter
...
func setupLoopback(con *os.File) {
	...
	if err != nil {
		consoleLog(con, "nexus-agent: network: lo: socket: %v\n", err)
		return
	}
	defer unix.Close(fd)
	...
	if err != nil {
		consoleLog(con, "nexus-agent: network: lo: NewIfreq: %v\n", err)
		return
	}
	// Read current flags.
	if err := unix.IoctlIfreq(fd, unix.SIOCGIFFLAGS, ifreq); err != nil {
		consoleLog(con, "nexus-agent: network: lo: SIOCGIFFLAGS: %v\n", err)
		return
	}
	...
	if err := unix.IoctlIfreq(fd, unix.SIOCSIFFLAGS, ifreq); err != nil {
		consoleLog(con, "nexus-agent: network: lo: SIOCSIFFLAGS: %v\n", err)
		return
	}
	consoleLog(con, "nexus-agent: network: lo: up\n")
}

// setupNetwork configures the guest virtio-net interface for the nexus
...
func setupNetwork(con *os.File) {
	...
	if err := os.WriteFile("/etc/resolv.conf", []byte(dns), 0o644); err != nil {
		consoleLog(con, "nexus-agent: network: write /etc/resolv.conf: %v\n", err)
	...
	// when nexus-agent is PID 1).
	iface := firstNonLoIface()
	if iface == "" {
		consoleLog(con, "nexus-agent: network: no non-loopback interface found; skipping\n")
		return
	}
	consoleLog(con, "nexus-agent: network: configuring %s: ip=%s gw=%s\n",
	...
	// nexus-agent runs as PID 1, so kernel init PATH may omit these dirs;
	...
	if ipBin == "" {
		consoleLog(con, "nexus-agent: network: ip(8) not found at /usr/sbin/ip, /sbin/ip, /bin/ip; skipping\n")
		return
	}
	...
	consoleLog(con, "nexus-agent: network: %s configured\n", iface)
...
func checkEgressWith(iface, sysRoot string, probe func(addr string, timeout time.Duration) error, con *os.File) bool {
	...
	if !hwBacked || !dnsOK {
		consoleLog(con,
			"nexus-agent: EGRESS SELF-CHECK FAILED: iface=%s hwbacked=%v gateway=%s dns=%s"+
				" — network egress is broken; in-guest builds/pulls will fail\n",
			iface, hwBacked, guestNetworkGateway, dnsStatus)
		return false
	}

	consoleLog(con, "nexus-agent: egress self-check ok (iface=%s)\n", iface)
	return true
}
