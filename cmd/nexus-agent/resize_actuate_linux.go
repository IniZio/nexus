...
func startCPUOnliner(ctx context.Context) {
	go func() {
		defer func() {
			if r := recover(); r != nil {
				fmt.Fprintf(os.Stderr, "nexus-agent: cpu-onliner: panic recovered: %v\n", r)
			}
		}()
...
func onlineOfflineCPUs() {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus-agent: cpu-onliner: ReadDir %s: %v\n", sampleCPUSysPath, err)
		return
	}
	for _, entry := range entries {
		...
		if err != nil {
			...
			fmt.Fprintf(os.Stderr, "nexus-agent: cpu-onliner: read %s: %v\n", onlinePath, err)
			continue
		}
		...
		if err := os.WriteFile(onlinePath, []byte("1"), 0); err != nil {
			fmt.Fprintf(os.Stderr, "nexus-agent: cpu-onliner: online %s: %v\n", name, err)
			continue
		}
