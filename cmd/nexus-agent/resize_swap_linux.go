...
//
// Normative MUST (D-DC-21): docs/site/sandboxes/execution-substrate.md §Resource limits.
...
// active; never fails boot. nexus's kernel has CONFIG_ZRAM=y
...
func setupZRAMSwap(con *os.File) {
	...
	if modprobe, err := exec.LookPath("modprobe"); err == nil {
		...
		if err != nil {
			consoleLog(con, "nexus-agent: zram-swap: modprobe zram skipped (non-fatal): %v: %s\n",
				err, strings.TrimSpace(string(out)))
		}
	}

	if !zramControlExistsFunc() {
		consoleLog(con, "nexus-agent: zram-swap: %s absent (CONFIG_ZRAM not set); swap safety net disabled\n",
			zramControlPath)
		return
	}
	if zramSwapActiveFunc() {
		consoleLog(con, "nexus-agent: zram-swap: swap already active; skipping setup\n")
		return
	}
	...
	if err != nil || totalKB == 0 {
		consoleLog(con, "nexus-agent: zram-swap: cannot read MemTotal (%v); skipping\n", err)
		return
	}
	...
	if err != nil {
		consoleLog(con, "nexus-agent: zram-swap: create device failed (non-fatal): %v\n", err)
		return
	}

	if out, err := zramExecFunc("mkswap", dev); err != nil {
		consoleLog(con, "nexus-agent: zram-swap: mkswap %s failed (non-fatal): %v: %s\n",
	...
	if out, err := zramExecFunc("swapon", "--priority", "100", dev); err != nil {
		consoleLog(con, "nexus-agent: zram-swap: swapon %s failed (non-fatal): %v: %s\n",
			dev, err, strings.TrimSpace(string(out)))
		return
	}
	if err := zramWriteFileFunc(swappinessPath,
		[]byte(strconv.Itoa(swapSafetyNetSwappiness)), 0o644); err != nil {
		consoleLog(con, "nexus-agent: zram-swap: set swappiness failed (non-fatal): %v\n", err)
	}
	consoleLog(con, "nexus-agent: zram-swap: enabled %s (%d MiB, swappiness=%d)\n",
