...
// are already running.  NEXUS_HOT_SWAP=1 is set in the exec env to signal
...
// (/sbin/nexus-agent on the rootfs ext4 partition).  Using /tmp would
// cause os.Rename to cross the tmpfs→ext4 device boundary and fail with
// EXDEV.  The staging path is /sbin/.nexus-agent.upgrade (same directory
// as the install path, guaranteed same device).
//
// Rollback
// ─────────
// Before overwriting the install path we rename it to
// /sbin/.nexus-agent.prev as a backup.  On any error AFTER the
...
//  6. Build env with NEXUS_HOT_SWAP=1 so the new process skips cold-boot init.
...
func performSwap(
	...
	// ── Step 6: build env ─────────────────────────────────────────────────
	// NEXUS_HOT_SWAP=1 signals the new process image to skip cold-boot init
	...
	env := os.Environ()
	env = append(env, "NEXUS_HOT_SWAP=1")
