...
func TestPerformSwap_HappyPath(t *testing.T) {
	...
	err := performSwap(fns, "/sbin/.nexus-agent.upgrade", 12345, "/sbin/nexus-agent", "/sbin/.nexus-agent.prev")
	if err != nil {
	...
	for i, e := range seq {
		switch {
		case e.tag == "rename:/sbin/.nexus-agent.upgrade->/sbin/nexus-agent":
			installRenameIdx = i
		// chmod must target the install path, not the staged path.
		case e.tag == "chmod:/sbin/nexus-agent":
			chmodInstallIdx = i
	...
	// chmod must target exactly the install path (/sbin/nexus-agent), not the staged path.
	if chmodInstallIdx == -1 {
		t.Errorf("chmod:/sbin/nexus-agent not found; seq=%v", seq)
	}
	if execIdx == -1 {
...
// ─────────────────────────────────────────────────────────────────────────────
// Mutation proof: NEXUS_HOT_SWAP=1 must appear in the exec env.
//
// Production path: performSwap → execSelf(argv, env).
// This test captures the env passed to execSelf and verifies the signal is
// present. The mutation is: delete or blank the NEXUS_HOT_SWAP line in
// performSwap → execSelf receives env without it → test fails.
//
// Also verifies the old fd-inheritance vars (NEXUS_CTRL_FD, NEXUS_DATA_FD)
...
func TestPerformSwap_HotSwapEnvSignal(t *testing.T) {
	...
	// NEXUS_HOT_SWAP=1 must be present so the new process skips cold-boot init.
	foundHotSwap := false
	for _, kv := range capturedEnv {
		if kv == "NEXUS_HOT_SWAP=1" {
			foundHotSwap = true
		}
		// Verify the removed fd-inheritance vars are absent.
		if len(kv) >= 12 && kv[:12] == "NEXUS_CTRL_" {
			t.Errorf("NEXUS_CTRL_FD must not appear in exec env; got %q", kv)
		}
		if len(kv) >= 12 && kv[:12] == "NEXUS_DATA_" {
			t.Errorf("NEXUS_DATA_FD must not appear in exec env; got %q", kv)
		}
	}
	if !foundHotSwap {
		t.Errorf("NEXUS_HOT_SWAP=1 not found in exec env; env had %d entries", len(capturedEnv))
	}
}
...
func TestPerformSwap_ChmodFails(t *testing.T) {
	...
	sawRollback := false
	for _, c := range *calls {
		if c == "rename:/sbin/.nexus-agent.prev->/sbin/nexus-agent" {
			sawRollback = true
		}
	}
	if !sawRollback {
...
func TestPerformSwap_FsyncCalledBeforeRename(t *testing.T) {
	...
	_ = performSwap(fns, "/sbin/.nexus-agent.upgrade", 12345, "/sbin/nexus-agent", "/sbin/.nexus-agent.prev")
	...
	for i, e := range seq {
		...
		if e.kind == "rename" && e.arg == "/sbin/.nexus-agent.upgrade->/sbin/nexus-agent" {
			installRenameIdx = i
		}
	}
...
func TestRenameAcrossDeviceFails(t *testing.T) {
	...
	tmpfsFile := filepath.Join("/dev/shm", "nexus-rename-test-staged")
	...
	rootfsTarget := filepath.Join(varTmp, "nexus-rename-test-target")
	...
	sameDeviceStaged := filepath.Join(varTmp, "nexus-rename-test-staged")
...
func TestControlServer_RestartAgent_ActiveSessionsNoForce(t *testing.T) {
	...
	_, err := cs.RestartAgent(nil, &agentpb_RestartAgentRequest{
		StagedPath:    "/sbin/.nexus-agent.upgrade",
		ExpectedBytes: 100,
		Force:         false,
	})
	if err == nil {
