...
func runGitSSHShim(args []string) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus-agent git-ssh: dial host relay (vsock CID 2 port %d): %v\n",
			driver.GitSSHRelayPort, err)
		os.Exit(1)
	}
	code, err := execGitSSHShim(args, conn)
	if err != nil {
		fmt.Fprintf(os.Stderr, "nexus-agent git-ssh: %v\n", err)
		os.Exit(1)
	}
	os.Exit(int(code))
}
...
func execGitSSHShim(args []string, conn net.Conn) (int32, error) {
	...
	go func() { // frame reader: stdout data and exit frame from host relay
		for {
			...
			if err != nil {
				if !isShimEOF(err) {
					fmt.Fprintf(os.Stderr, "nexus-agent git-ssh: read frame: %v\n", err)
				}
			...
			case gitssh.FrameTypeExit:
				...
				return
			default:
				fmt.Fprintf(os.Stderr, "nexus-agent git-ssh: unknown frame type 0x%02x; ignoring\n", ft)
			}
		}
	}()
