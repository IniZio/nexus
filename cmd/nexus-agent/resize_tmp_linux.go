...
// in the slice on that basis. THAT IS FALSE FOR NEXUS, and it is why the
...
// source and never re-checked here. In nexus, buildkitd's --root is
...
//
// TBD-DC-9 (boot ceiling seam) settlement: nexus uses the kernel cmdline
...
func startTmpfsResizer(ctx context.Context, con *os.File) {
	go func() {
		defer func() {
			if r := recover(); r != nil {
				consoleLog(con, "nexus-agent: tmp-resize: panic recovered: %v\n", r)
			}
		}()

		if err := resizeTmpfsOnce(con); err != nil {
			consoleLog(con, "nexus-agent: tmp-resize: %v\n", err)
		}
...
func resizeTmpfsOnce(con *os.File) error {
	...
	consoleLog(con, "nexus-agent: tmp-resize: /tmp grown %d MiB → %d MiB\n",
		cur>>20, target>>20)
	return nil
}
