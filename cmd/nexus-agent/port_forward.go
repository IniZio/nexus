...
func startPortForwardMux(ctx context.Context, con *os.File) {
	...
	if err != nil {
		consoleLog(con, "nexus-agent: port-forward-mux: vsock.Listen %d: %v\n", portForwardMuxVsockPort, err)
		return
	}
	...
	consoleLog(con, "nexus-agent: port-forward-mux: listening on vsock port %d\n", portForwardMuxVsockPort)
	...
	for {
		...
		if err != nil {
			...
			consoleLog(con, "nexus-agent: port-forward-mux: accept: %v\n", err)
			return
		}
...
func handlePortForward(con *os.File, vsockConn net.Conn) {
	...
	if _, err := io.ReadFull(vsockConn, portBuf[:]); err != nil {
		consoleLog(con, "nexus-agent: port-forward-mux: read port: %v\n", err)
		return
	}
	...
	if err != nil {
		consoleLog(con, "nexus-agent: port-forward-mux: dial :%d: %v\n", guestPort, err)
		return
	}
	defer tcpConn.Close()
