...
func main() {
	...
	hotSwap := os.Getenv("NEXUS_HOT_SWAP") != ""
	os.Unsetenv("NEXUS_HOT_SWAP")
	...
	if hotSwap {
		consoleLog(con, "nexus-agent: hot-swap boot (pid=%d build=%s); skipping cold-boot init\n", os.Getpid(), agentBuildTag)
	} else {
		consoleLog(con, "nexus-agent: starting (pid=%d build=%s)\n", os.Getpid(), agentBuildTag)
	}
	if isPid1 && !hotSwap {
		consoleLog(con, "nexus-agent: /tmp: RAM-backed tmpfs (32 MiB seed; resizer target = max(1 GiB, min(50%%%% MemTotal, 2 GiB)))\n")
	}
		...
		for _, arg := range os.Args[1:] {
			...
			case strings.HasPrefix(arg, "--scratch-disk="):
				if dev, ok := parseScratchDiskArg(arg); ok {
					...
					consoleLog(con, "nexus-agent: ignoring malformed --scratch-disk arg: %q\n", arg)
				}
			case strings.HasPrefix(arg, "--mem-ceiling="):
				...
				if n, err := strconv.ParseInt(v, 10, 64); err == nil {
					...
					consoleLog(con, "nexus-agent: ignoring malformed --mem-ceiling arg: %q\n", arg)
				}
			case strings.HasPrefix(arg, "--cache-disk="):
				...
				if len(parts) == 2 && parts[0] != "" && parts[1] != "" {
					...
					consoleLog(con, "nexus-agent: ignoring malformed --cache-disk arg: %q\n", arg)
				}
			...
			case strings.HasPrefix(arg, "--workspace-mount="):
				...
				if ok {
					...
					consoleLog(con, "nexus-agent: ignoring malformed --workspace-mount arg: %q\n", arg)
				}
			default:
				consoleLog(con, "nexus-agent: WARN: unrecognized cmdline arg %q — host/guest version skew? agent build=%s\n", arg, agentBuildTag)
			}
		}
	}
	...
	// Set guest hostname from --sandbox-handle= or "nexus"
	if isPid1 {
		hn := sandboxHandle
		if hn == "" {
			hn = "nexus"
		...
		if err := syscall.Sethostname([]byte(hn)); err != nil {
			consoleLog(con, "nexus-agent: sethostname(%q): %v\n", hn, err)
		} else {
	...
	coldBootCfg := bootConfig{
		...
		mountWorkspace: func(mounts []agent.GuestMount) error {
			consoleLog(con, "nexus-agent: mounting workspace (%d mounts)\n", len(mounts))
	...
	if err := runColdBootInit(hotSwap, isPid1, wsMounts, scratchDev, coldBootCfg, nil); err != nil {
		consoleFatal(con, isPid1, "nexus-agent: workspace mount failed: %v\n", err)
	}
	...
	if isBuilderRole && len(resizableDisks) > 0 {
		consoleLog(con, "nexus-agent: builder role: resize telemetry: %d cache disk(s)\n", len(resizableDisks))
	}

	// Auto-resize: PID-1 regular-agent only (builder child cannot rebind vsock:3002)
	if !isBuilderRole {
		consoleLog(con, "nexus-agent: auto-resize: starting services (mem-ceiling=%d)\n", memCeilingBytes)
		startResizeServices(ctx, con, resizableDisks, memCeilingBytes)
	}

	// Builder-role: run build and exit
	if isBuilderRole {
		consoleLog(con, "nexus-agent: builder role starting (cache disks: %d)\n", len(cacheDiskMounts))
		...
		if err != nil {
			consoleFatal(con, isPid1, "nexus-agent: builder role: %v\n", err)
		}
		opts.ToolRecipe = recipe
		if err := agent.RunBuilderRole(ctx, opts); err != nil {
			consoleFatal(con, isPid1, "nexus-agent: builder role: %v\n", err)
		}
		consoleLog(con, "nexus-agent: builder role complete\n")
		os.Exit(0)
	}
	...
	{
		var err error
		consoleLog(con, "nexus-agent: vsock.Listen port %d\n", driver.AgentControlPort)
		ctrlLis, err = vsock.Listen(driver.AgentControlPort, nil)
		if err != nil {
			consoleFatal(con, isPid1, "nexus-agent: control listener (port %d): %v\n",
				driver.AgentControlPort, err)
		}
		consoleLog(con, "nexus-agent: control plane listening\n")

		consoleLog(con, "nexus-agent: vsock.Listen port %d\n", wire.DataPort)
		dataLis, err = vsock.Listen(wire.DataPort, nil)
		if err != nil {
			consoleFatal(con, isPid1, "nexus-agent: data listener (port %d): %v\n",
				wire.DataPort, err)
		}
	}
	consoleLog(con, "nexus-agent: data plane listening; running agent\n")
	...
	if err := a.Run(ctx); err != nil {
		consoleFatal(con, isPid1, "nexus-agent: run: %v\n", err)
	}
	consoleLog(con, "nexus-agent: clean shutdown\n")
}
...
func mountGuestFS() {
	tryMount := func(src, target, fstype, data string) {
		if err := guestMountFn(src, target, fstype, 0, data); err != nil && err != syscall.EBUSY {
			fmt.Fprintf(os.Stderr, "nexus-agent: mount %s: %v\n", target, err)
		}
	}
	...
	if _, err := os.Stat("/dev/ptmx"); os.IsNotExist(err) {
		if err := unix.Mknod("/dev/ptmx", unix.S_IFCHR|0o666,
			...
			fmt.Fprintf(os.Stderr, "nexus-agent: mknod /dev/ptmx: %v\n", err)
		}
	}

	if err := os.MkdirAll("/dev/pts", 0o755); err != nil {
		fmt.Fprintf(os.Stderr, "nexus-agent: mkdir /dev/pts: %v\n", err)
	} else {
		tryMount("devpts", "/dev/pts", "devpts", "")
	}

	if err := os.MkdirAll("/dev/shm", 0o755); err != nil {
		fmt.Fprintf(os.Stderr, "nexus-agent: mkdir /dev/shm: %v\n", err)
	} else {
	...
	if err := os.MkdirAll("/sys/fs/cgroup", 0o755); err != nil {
		fmt.Fprintf(os.Stderr, "nexus-agent: mkdir /sys/fs/cgroup: %v\n", err)
	} else {
