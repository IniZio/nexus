...
func TestExecStreamAndReattach(t *testing.T) {
	...
	if !bytes.Contains(out1, []byte("NEXUS")) {
		t.Fatalf("expected 'NEXUS' in output, got %q", out1)
	}
...
func TestStdinForwarding(t *testing.T) {
	...
	var got []byte
	for !bytes.Contains(got, []byte("hello nexus")) {
		f, err := r.ReadFrame()
		if err != nil {
			break
		}
	...
	if !bytes.Contains(got, []byte("hello nexus")) {
