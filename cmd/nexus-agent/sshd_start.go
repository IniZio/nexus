...
func startSSHD(con *os.File) {
	...
	if err := os.MkdirAll("/run/sshd", 0o755); err != nil {
		consoleLog(con, "nexus-agent: sshd: mkdir /run/sshd: %v\n", err)
		// Non-fatal — attempt to start sshd anyway.
	}

	// Normalize /root ownership so sshd StrictModes passes.
	// nexus images are built by rootless buildkit (userns subuid mapping), so
	...
	if err := os.Chown("/root", 0, 0); err != nil {
		consoleLog(con, "nexus-agent: sshd: chown /root: %v\n", err)
		// Non-fatal — attempt to start sshd anyway.
	}
	if _, err := os.Stat("/root/.ssh"); err == nil {
		if err := os.Chown("/root/.ssh", 0, 0); err != nil {
			consoleLog(con, "nexus-agent: sshd: chown /root/.ssh: %v\n", err)
		}
	}
	...
	if err := cmd.Start(); err != nil {
		consoleLog(con, "nexus-agent: sshd: start failed: %v\n", err)
		return
	}
	// Detach: we don't own the daemon's lifecycle; PID 1 will reap it.
	_ = cmd.Process.Release()
	consoleLog(con, "nexus-agent: sshd: started (pid=%d)\n", cmd.Process.Pid)
}
