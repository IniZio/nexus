...
//
//  1. All .go files in cmd/nexus-agent/ for non-comment lines that either:
...
//
//   - cmd/nexus-agent/main.go:436 tryMount("tmpfs", "/tmp", …): the call
//     that currently creates the tmpfs mount; Wave-2 changes this.
//
//   - cmd/nexus-agent/mount_linux_test.go:36 c.fstype == "tmpfs" for /tmp:
...
func TestNoTmpfsAssumptionOnSlashTmp(t *testing.T) {
	...
	// thisFile = …/cmd/nexus-agent/tmpfs_assumption_sweep_test.go
...
// real-package RED→GREEN proof (writing a probe into cmd/nexus-agent/ and
