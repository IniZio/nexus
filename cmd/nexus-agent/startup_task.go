...
// executable here (e.g. a .nexus/Containerfile that writes it). It is nexus's
...
var startupHookPath = "/etc/nexus/startup"
...
func runStartupHook(con *os.File) {
	...
	case startupSkipNonExecutable:
		consoleLog(con, "nexus-agent: startup hook %s present but not executable; skipping\n", path)
		return
	}

	consoleLog(con, "nexus-agent: startup hook %s: running (background, non-fatal)\n", path)
	go func() {
		...
		if len(out) > 0 {
			consoleLog(con, "nexus-agent: startup hook output:\n%s", out)
		}
		if err != nil {
			consoleLog(con, "nexus-agent: startup hook exited: %v (non-fatal; sandbox continues)\n", err)
			return
		}
		consoleLog(con, "nexus-agent: startup hook complete\n")
	}()
}
...
func runBootTask(con *os.File, task bootspec.Task) {
	if len(task.Argv) == 0 {
		...
		consoleLog(con, "nexus-agent: boot task %q: empty argv; skipping\n", name)
		return
	}
	label := task.Name
	if label == "" {
	...
	if err != nil {
		consoleLog(con, "nexus-agent: boot task %q: %v; skipping\n", label, err)
		return
	}
	...
	launch := func() {
		...
		out, err := execCollectCmd(cmd)
		if len(out) > 0 {
			consoleLog(con, "nexus-agent: boot task %q output:\n%s", label, out)
		}
		if err != nil {
			consoleLog(con, "nexus-agent: boot task %q exited: %v (non-fatal; sandbox continues)\n", label, err)
		} else {
			consoleLog(con, "nexus-agent: boot task %q complete\n", label)
		}
	}
...
// (/etc/nexus/boot.json) and runs each declared task in order. A missing or
// unparseable manifest falls back to the legacy /etc/nexus/startup behavior.
//
...
func runBootTasks(con *os.File) {
	...
	if err := json.Unmarshal(data, &spec); err != nil {
		consoleLog(con, "nexus-agent: boot.json unparseable: %v; falling back to startup hook\n", err)
		runStartupHook(con)
		return
	}
