...
func TestParseSupervisorFlags_RoundTrip(t *testing.T) {
	in := supervisor.Config{
		...
		Cmdline: "root=/dev/vda rw init=/sbin/nexus-agent console=ttyS0 -- --workspace-mount=/dev/vdf:/workspace/proj:ext4:false:true",
...
// nexus) must deserialise to NestedVirt=false, never NestedVirt=true.
//
...
// supervisor is launched as `nexus __supervisor <argv>`, and the argv codec is
