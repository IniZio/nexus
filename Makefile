...
# `go build ./cmd/nexus` into a temp dir), and a nested `go build` is a FRESH
...
# (or `nexus kernel install`) which downloads the released binary.
NEXUS_DATA_DIR ?= $(if $(XDG_DATA_HOME),$(XDG_DATA_HOME),$(HOME)/.local/share)/nexus
NEXUS_KERNEL_DST := $(NEXUS_DATA_DIR)/images/kernel/vmlinux-x86_64
NEXUS_KERNEL_SRC := $(CURDIR)/images/kernel/vmlinux-x86_64

install-kernel:
	@mkdir -p $(dir $(NEXUS_KERNEL_DST))
	ln -sf $(NEXUS_KERNEL_SRC) $(NEXUS_KERNEL_DST)
	@echo "OK: kernel symlinked → $(NEXUS_KERNEL_DST) → $(NEXUS_KERNEL_SRC)"

# install-kernel-release: download the kernel matching the installed nexus
# binary via `nexus kernel install`. Requires a released binary (fails for
# -dev builds because --version is required). For dev use, use install-kernel.
install-kernel-release:
	nexus kernel install

# build-agent: legacy alias — installs to NEXUS_AGENT_INSTALL_DIR (same as install-agent).
# Previously wrote to /tmp; use install-agent for new scripts.
build-agent: install-agent

# install-plugin: register this checkout as the "nexus" marketplace (root
...
install-plugin:
	@claude plugin marketplace list 2>/dev/null | grep -q '^  ❯ nexus$$' || \
...
# nothing — which is exactly what happened once, hiding a real cmd/nexus-agent
...
#
#   Part 2 — on-PATH builder agent (resolved via exec.LookPath("nexus-agent")):
#     Used ONLY by `nexus create --file` to bake a custom builder VM image.
...
check-agent-fresh:
	@bin=images/kernel/nexus-agent; \
	if [ ! -f $$bin ]; then \
		echo "FAIL: $$bin missing — run images/kernel/rebuild-base.sh"; exit 1; \
	fi; \
	stale=$$(find cmd/nexus-agent internal/core/agent -name '*.go' -newer $$bin 2>/dev/null | head -1); \
	if [ -n "$$stale" ]; then \
		echo "FAIL: agent source newer than $$bin (first offender: $$stale)"; \
		echo "      the nexus-agent-base image embeds a stale agent — runtime failures are silent"; \
	...
	if [ -z "$$path_bin" ]; then \
		...
		stale2=$$(find cmd/nexus-agent internal/core/agent -name '*.go' -newer "$$path_bin" 2>/dev/null | head -1); \
		if [ -n "$$stale2" ]; then \
			...
			echo "      silently re-baked — new agent features never run even after \`go build ./cmd/nexus\`"; \
			echo "      fix: make install-agent"; \
			exit 1; \
		fi; \
...
secret:
	@echo "nexus keeps no committed secrets. Credentials are brokered at runtime via the agent credential subsystem."
