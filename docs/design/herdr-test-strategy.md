...
This is the standing contract for how the herdr+nexus integration is tested.
...
Any assertion about an interface nexus does **not** own requires a Layer 4
...
- **herdr's CLI surface** — a flag name nexus passes, a subcommand nexus
  calls, or an argv shape nexus constructs is not owned by nexus. Layers 1–3
  stub the exec seam; they accept any argv without complaint. Layer 4 must
  assert the real herdr binary accepts the exact argv nexus sends.

- **herdr's JSON response shape** — a field name nexus parses from herdr
  output is not owned by nexus. A herdr upgrade that renames a field is
...
- **On-disk binding format** — a field nexus reads from a binding record
...
every argv the scripts would pass to the `nexus` binary, and one that records
...
| `TestOpenPaneScript_OverlayWithEnvOmitsWorkspace` | When `NEXUS_WORKSPACE` is set with an overlay placement, `--env NEXUS_WORKSPACE=…` is forwarded but `--workspace` is still absent — the server rejects `--workspace` for overlay regardless of the env var. |
| `TestOpenPaneScript_OmitsEnvFlagWhenWorkspaceUnset` | When `NEXUS_WORKSPACE` is not set, `--env "NEXUS_WORKSPACE="` is NOT forwarded (an empty ref is not the same as an absent one). |
| `TestOpenPaneScript_PassesEnvFlagWhenWorkspaceSet` | When `NEXUS_WORKSPACE` is set, `--env NEXUS_WORKSPACE=<value>` reaches herdr. |
...
| `TestPaneScript_ShellRefusesWithoutWorkspace` | The shell pane exits non-zero and names `NEXUS_WORKSPACE` in the error when the variable is unset. |
...
and the CLI argument surfaces of both `nexus` and `herdr` is correct for every
branching condition in the scripts.

**What this layer cannot prove:** That the real `nexus` binary accepts the argv
...
Calls `runHerdrPlugin` directly — the same Go function that `nexus
...
**Mechanism:** `TestHerdrPlugin_L4_BinaryVerb` builds the nexus binary, then
makes two probes:

1. **Direct exec** — calls `nexus herdr abi` as a subprocess and
...
- **Command acceptance** — every `herdr` subcommand nexus invokes is verified
  ...
  each flag nexus passes appears in the usage text. A flag-name typo (e.g.
...
**What `TestHerdrPlugin_L4_Contract` proves:** Every herdr command nexus
...
end-to-end; or that optional flags behave as nexus expects when present.
...
`herdr plugin action invoke` with the nexus plugin active and assert on
the resulting pane.
