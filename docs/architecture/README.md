...
nexus3 runs workloads in microVM sandboxes — real Linux VMs with their own kernel, disk image,
...
```mermaid
C4Context
  title nexus3 — System Context

  Person(dev, "Developer / Orchestrator", "Runs nexus3 CLI or connects via MCP")

  System(nexus3, "nexus3", "MicroVM sandbox runtime — creates isolated Linux VMs, routes egress, brokers credentials")
  ...
  System_Ext(upstream, "GitHub / upstream internet", "Package registries, GitHub API, LLM endpoints — egress filtered by nexus3 perimeter")

  Rel(dev, nexus3, "sandbox create / exec / snapshot / fork", "CLI flags or MCP JSON-RPC")
  Rel(nexus3, herdr, "opens panes, dispatches briefs", "herdr CLI sub-commands")
  Rel(nexus3, kvm, "spawns cloud-hypervisor process per sandbox", "exec + KVM fd")
  Rel(claude, nexus3, "gRPC over vsock (exec, copy, resize)", "virtio-vsock")
  Rel(nexus3, upstream, "proxied egress; blocked by allowlist", "MITM HTTPS proxy")
  Rel(dev, herdr, "opens workspaces, views panes", "herdr CLI / UI")
  Rel(herdr, nexus3, "herdr plugin calls nexus3 MCP server", "MCP stdio")
```
...
```mermaid
C4Container
  title nexus3 — Containers

  Person(dev, "Developer / Orchestrator")

  Container(cli, "nexus3 CLI", "Go binary — cmd/nexus3", "Parses flags, calls Service, drives lifecycle")
  ...
  Container(supervisor, "Detached Supervisor", "Go — internal/supervisor (re-exec of nexus3 binary)", "One process per sandbox; owns cloud-hypervisor child, perimeter, watchdog pipe")
  ...
  Container(agent, "Guest Agent", "Go — cmd/nexus3-agent (static binary in guest)", "PID-1-ish in VM; gRPC/vsock server; exec/copy/resize/hotswap")

  System_Ext(kvm, "cloud-hypervisor binary", "KVM VMM process")
  System_Ext(upstream, "GitHub / upstream internet")
  System_Ext(herdr, "herdr")

  Rel(dev, cli, "nexus3 sandbox create / exec / …", "stdin/stdout")
...
|-----------|-----------------|
| nexus3 CLI | `cmd/nexus3`, `internal/cli` |
...
| Detached Supervisor | `internal/supervisor` (re-exec of `cmd/nexus3` binary with `--supervisor` flag) |
...
| Guest Agent | `cmd/nexus3-agent`, `internal/core/agent/agentpb` (gRPC proto), `internal/clientagent` |

## Request flow: `nexus3 sandbox create`
...
3. **Service** calls `supervisor.SpawnDetached` (`internal/supervisor/spawn_linux.go`), which
   re-execs the nexus3 binary in a new session; the child writes a pidfile and signals readiness
   over a Unix socket.
...
6. **Guest Agent** (`cmd/nexus3-agent`) starts inside the VM, registers on vsock port 1024,
