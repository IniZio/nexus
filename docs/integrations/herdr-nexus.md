# herdr ↔ nexus integration

herdr is a terminal workspace manager. nexus ships a native herdr plugin
(`plugins/herdr/`) that maps each nexus sandbox to a herdr workspace whose
...
Every nexus sandbox gets exactly one herdr workspace (a "space").  The
...
**The space label** follows the convention `nexus:<sandbox-handle>`, e.g.
`nexus:demo-orca-01`.  The label is set at creation time and is stable for
...
Creating a space is a two-step operation. nexus never closes a tab or pane it
did not itself just open — see "Why nexus never closes a tab" below — so the
...
```sh
WS_JSON=$(herdr workspace create --label 'nexus:<sandbox-handle>' --no-focus --cwd '<sandbox-host-mount-path>')
...
herdr plugin pane open \
  ...
  --direction right \
  --env NEXUS_WORKSPACE=<sandbox-ref> \
  --focus
```
...
failure: nexus still never closes anything to compensate for it.
...
`--target-pane`. nexus adopts the same stance: it grafts, it never destroys a
...
type  = "plugin_action"
command = "nexus:shell"           # action id from herdr-plugin.toml [[actions]] id
```
...
> above to match the new action id (format: `nexus:<action-id>`).

---

## Server-side-only deployment

The nexus herdr plugin is registered and executed **on the engine host** (e.g.
...
herdr --remote engine-03 plugin pane open --plugin nexus --entrypoint shell \
  --workspace "$WS_ID" --env NEXUS_WORKSPACE=<sandbox-ref>
```

Plugin resolution, pane process spawning, and all `nexus exec` calls happen
server-side.  The `--remote` flag is a pure transport; no nexus binary, no
...
|---------------|--------------|
| `nexus sandbox create` | `herdr workspace create --label nexus:<handle> --cwd <host-mount-path>`, then pane open grafted onto the root pane (`--target-pane`); nothing is ever closed |
| `nexus sandbox pause`  | Panes become idle (exec returns); workspace persists |
| `nexus sandbox resume` | Re-open panes or reconnect existing ones |
| `nexus sandbox remove` | `herdr workspace close <ws-id>` (binding removed from store) |
...
- **New tabs opened by the user launch a HOST shell, not a guest shell.**
  When the user opens a new tab in a nexus space directly through herdr
  (rather than via the `shell` action / `space-open-pane`), herdr has no way
  to know that tab should run `nexus exec <sandbox-ref> /bin/sh` instead of
  a plain host shell. This is **not fixable in nexus**: the herdr plugin ABI
  ...
  addition); nexus cannot work around it from its side of the plugin
  boundary.
- **New tabs still do not open in the project directory, even with `--cwd`
  ...
  pane's directory, not the workspace's or root pane's. The nexus guest
  ...
  known gap, not something this integration fixes; there is no nexus-side
