...
Repos that already have a `.nexus/config.yaml` are pre-configured. A worktree sandbox reads the file from its own checkout — the same file `nexus3 create --file` builds from — so a change takes effect on the next worktree-sandbox create for that checkout; no push to the default branch is needed.
