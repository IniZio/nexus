...
**Risk**: The virtiofs transport that delivers the host worktree to the guest carries elevated metadata overhead compared to a virtio-blk disk. Measured values (D-PD-102/103, n=10 bench-redo on equal-footing `cp -a` copies, `auto` cache mode): `git status` is **4.74× slower** than ext4 in-guest (149.5 ms vs 31.5 ms, ratio of means) and **~10× slower** than the 14.75 ms host-native baseline; metadata **writes** remain **~10.8–14.6×** slower (create 14.6×, unlink 10.8×). These figures come from `2026-08-18-gitstatus-redo-auto-bench_lines.txt` (in-guest) and `2026-08-18-host-baseline.txt` (host-native baseline); both are local-only proof-of-concept capture files retained on the operator's machine and are not committed to the repo. The analysis and derived figures are documented in `docs/design/virtiofs-vs-ext4.md`. A separate always-writeback run (see `docs/design/virtiofs-vs-ext4.md`) showed 156.3 ms vs 34.5 ms (~4.5×) — consistent direction; the auto-cache-mode figure is reported here because it is what production gets: `ch_virtiofs.go` passes no `--cache` flag at all (argv is `--shared-dir`, `--socket-path`, `--sandbox none`, `--seccomp none`, optional `--readonly`), and `auto` is virtiofsd's upstream default. Note this holds by third-party default, not by anything this repo asserts — no test pins it, so a virtiofsd default change would silently move production off this figure. The earlier figure of "327 ms vs 16 ms (~20×)" was void — an `mke2fs -d` inode artifact caused git to re-hash 489 MB on one leg only. Build steps that create many small files would be penalized if run on the mounted path.
